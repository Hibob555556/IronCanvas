use iron_canvas::{CENTER, Vertex, rectangle_vertices, rotate_rectangle_z, rotate_z};
...
#[test]
fn rectangle_vertices_are_closed_and_have_expected_count() {
    let vertices = rectangle_vertices();

    assert_eq!(vertices.len(), 11);
    assert_eq!(vertices.first().unwrap().position, [0.0, 0.0, 0.0]);
    assert_eq!(vertices.last().unwrap().position, [0.0, 0.0, 0.0]);
}

#[test]
fn rectangle_vertices_use_expected_bounds() {
    let vertices = rectangle_vertices();
